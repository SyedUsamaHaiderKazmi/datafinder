...
#### Fixed
- input layout for filters had an array syntax error at html attribute `data-date-format`.
...
#### Fixed
- shifted from `fn() => []` `function(){return;}` for downgrade support for `php7.1-7.3`.
...
#### Added
- Enabled 3 more type support for filters, `date`, `time`, `month`.
...
#### Added
- A new approach to orderable column, if a column value is generating through model appends, then using `column_name` key instead of `data`, we can now order not just raw values from database, but also the appended values with this.
